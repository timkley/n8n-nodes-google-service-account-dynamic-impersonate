...
export class GoogleDynamicImpersonateApi implements ICredentialType {
	name = 'googleDynamicImpersonateApi';

	displayName = 'Google Service Account (Dynamic Impersonate) API';
	...
	properties: INodeProperties[] = [
		...
		},
		{
			...
			description:
				'The email address of the user for which the application is requesting delegated access. May be overwritten by the HTTP node by providing the email as `delegated-email` header.',
		},
		{
		...
		{
			displayName:
				"When using the HTTP Request node, you must specify the scopes you want to send as comma separated values. In other nodes, they're added automatically.",
			name: 'httpWarning',
			type: 'notice',
			default: '',
			displayOptions: {
	...
	async authenticate(
		...
		const sub = requestOptions.headers?.['delegated-email'] || credentials.delegatedEmail || credentials.email;
